...
class ItsmProfile(Base):
    ...
    __table_args__ = (
        CheckConstraint("platform IN ('servicenow', 'jira')", name="valid_itsm_platform"),
        Index("ix_itsm_profiles_name", "name", unique=True),
    )
...
class WatcherConfig(Base):
    ...
    auto_create_jobs = Column(Boolean, nullable=False, default=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())

    __table_args__ = (
        Index("ix_watcher_configs_enabled", "enabled"),
    )


class WatcherEvent(Base):
    """Events emitted by filesystem watchers."""

    __tablename__ = "watcher_events"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    watcher_id = Column(UUID(as_uuid=True), ForeignKey("watcher_configs.id", ondelete="SET NULL"), index=True)
