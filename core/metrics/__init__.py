...
from .models import ArchiveGuardrailMetricEvent, DetectionMetricEvent
...
def setup_metrics() -> None:
    ...
    """
    global _metrics_configured, _metrics_server
    ...
    if _metrics_server is None:
        try:
            ...
            logger.info(
                "Prometheus metrics server listening on http://0.0.0.0:%s/metrics",
                settings.METRICS_PORT,
            )
        except OSError as exc:  # pragma: no cover - port conflicts should not break startup
            logger.warning(
                "Unable to start dedicated metrics server on port %s: %s",
                settings.METRICS_PORT,
                exc,
            )
...
from .collectors import observe_archive_guardrail, observe_detection  # noqa: E402  pylint: disable=wrong-import-position
